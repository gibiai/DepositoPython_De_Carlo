...
# classe basa astratta come modello per le sottoclassi
class Piatto(ABC):
    def __init__(self, codice, nome, categoria, prezzo):
        self.codice = codice
        self.nome = nome
    ...
    def __str__(self): # metodo speciale - rappresentazione testuale del piatto per file txt - restituisce stringa leggibile
        return f"[{self.codice}] {self.get_tipo().upper()} - {self.nome} | €{self.prezzo:.2f}"
...
class Antipasto(Piatto):
    ...
    def descrivi(self):
        print(f"[{self.codice}] ANTIPASTO - {self.nome} | €{self.prezzo:.2f}")
...
class Primo(Piatto):
    ...
    def descrivi(self):
        print(f"[{self.codice}] PRIMO - {self.nome} | €{self.prezzo:.2f}")
...
class Secondo(Piatto):
    ...
    def descrivi(self):
        print(f"[{self.codice}] SECONDO - {self.nome} | €{self.prezzo:.2f}")
